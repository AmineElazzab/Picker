...
const render = async (elements) => {
    ...
    await elements.map((e) => {
        if (e.status === "notChoosed") {
            ...
            newSpin.innerHTML = `
            <div class="flex w-48 rounded-lg px-4 h-48 scale-50 text-base flex-col  shadow-lg shadow-white p-3 ${(e.id == random) ? 'bg-yellow-400 text-white h-52 -mt-2' : 'bg-white'} ${(e.status != 'notChoosed') ? 'bg-gray-200' : ''}" id="id_${e.id}" >
